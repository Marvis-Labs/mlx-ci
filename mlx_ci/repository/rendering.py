...
class BotSection:
    ...
    def render(self) -> list[str]:
        ...
        if self.stages:
            ...
            lines.extend(
                "| "
                f"{_cell(stage.name)} | {_cell(stage.status)} | "
                f"{_cell(stage.detail)} |"
...
class WorkOutput:
    """Render one independent section for every affected work subject."""

    phase_order = ("synthetic", "hf_checkpoint")

    def __init__(self, component_names: frozenset[str]):
        self.component_names = component_names

    def sections(self, record: Mapping[str, Any]) -> Sequence[BotSection]:
        return tuple(
            self._section(record, subject)
            for subject in sorted(self._subjects(record))
        )

    def _subjects(self, record: Mapping[str, Any]) -> set[str]:
        subjects = {
            _subject(item)
            for key in ("jobs", "gates", "results")
            for item in _items(record, key)
            if item.get("component") in self.component_names and _subject(item)
        }
        subjects.update(
            str(error["subject"])
    ...
    def _section(self, record: Mapping[str, Any], subject: str) -> BotSection:
        jobs = self._jobs(record, subject)
        gates = self._matching(record, "gates", subject)
        errors = self._errors(record, subject)
        results = self._matching(record, "results", subject)
        blocked = (
            record.get("kind") == "ci_execution" and record.get("outcome") == "blocked"
        )
        return BotSection(
            title=subject,
            component=_work_type((*jobs, *gates, *results, *errors)),
    ...
    def _matching(
        self, record: Mapping[str, Any], key: str, subject: str
    ) -> list[Mapping[str, Any]]:
        return [
            item
            for item in _items(record, key)
            if item.get("component") in self.component_names
            and _subject(item) == subject
        ]
    ...
    def _errors(
        ...
        return [
            ...
            and str(error.get("subject")) == subject
        ]

    def _jobs(
        self, record: Mapping[str, Any], subject: str
    ) -> list[Mapping[str, Any]]:
        jobs = self._matching(record, "jobs", subject)
        for gate in self._matching(record, "gates", subject):
            pending = gate.get("pending_work")
            if isinstance(pending, Mapping):
                jobs.append(pending)
        return list({str(job.get("id", subject)): job for job in jobs}.values())
    ...
    def _messages(
        ...
        if any(gate.get("status") == "awaiting_maintainer_approval" for gate in gates):
            messages.append(
                ...
                "maintainer approves this gated run."
            )
...
class BotOutput:
    ...
    def _marker(self) -> str:
        attempt = self.record.get("attempt_id")
        if self.record.get("kind") == "ci_execution" and attempt:
            return f"<!-- mlx-ci:attempt:{_cell(attempt)} -->"
        return "<!-- mlx-ci:plan -->"
...
def _work_type(items: Sequence[Mapping[str, Any]]) -> str:
    for item in items:
        if value := item.get("work_type"):
            return str(value)
        pending = item.get("pending_work")
        if isinstance(pending, Mapping) and pending.get("work_type"):
            return str(pending["work_type"])
    component = next((item.get("component") for item in items if item), "Work")
    return "".join(word.title() for word in str(component).split("_"))
...
def _failure_message(reason: str) -> str:
    ...
    messages = getattr(plugin, "failure_messages", lambda: {})()
    return messages.get(reason, "The work item failed.")
...
def _phase_name(value: str) -> str:
    return _display_labels().get(value, value.replace("_", " ").title())
...
def _display_labels() -> Mapping[str, str]:
    from ci import plugin

    return getattr(plugin, "display_labels", lambda: {})()
