...
def release_control(
    ...
    *,
    ...
    from mlx_ci.repository.execution_security import seal_job
...
def _validate_gate(gate: Mapping[str, Any], current_head_sha: str) -> None:
    ...
    from ci.plugin import validate_gate
...
def plan_repository(
    *,
    ...
    pr_number: int,
    rules_config: Path,
    component_config_directory: Path | None,
    protected_config: Path,
    run_url: str | None,
    output: Path,
    markdown: Path,
    ...
    try:
        ...
        plan = _blocked_plan(head, "invalid_ci_configuration", str(error))
    except subprocess.CalledProcessError as error:
        plan = _blocked_plan(head, "planner_failed", str(error))
    ...
    _write_record(record, output, markdown, github_output)
    return record
