...
BASE_SHA = "a" * 40
HEAD_SHA = "b" * 40
PARTICIPANT_ROOT = Path(__file__).parent / "fixtures" / "repository"
...
def test_released_job_accepts_bound_repository_identity():
    ...
    from mlx_ci.repository.execution_security import canonical_digest, validate_job
...
def test_status_renderer_is_centralized_and_suppresses_mentions():
    ...
    assert rendered.startswith("<!-- mlx-ci:plan -->")
...
def test_plan_repository_uses_immutable_repository_head(tmp_path):
    ...
    plan_repository(
        ...
        pr_number=8,
        rules_config=PARTICIPANT_ROOT / "ci/config/changes.yaml",
        component_config_directory=None,
        protected_config=PARTICIPANT_ROOT / "ci/config/protected-paths.yaml",
        run_url=None,
        output=output,
        markdown=markdown,
    )
...
def test_shared_export_turns_invalid_configuration_into_blocked_plan(
    ...
    monkeypatch.setattr("mlx_ci.repository.control.import_checkout", lambda *args: None)
    monkeypatch.setattr(
        "mlx_ci.repository.control.materialize",
